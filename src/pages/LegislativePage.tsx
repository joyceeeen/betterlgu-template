  ...
  const ordinanceSteps = [
    {
      ...
      desc: `Submit the proposed ordinance to the ${labels.legislativeBody} for consideration`,
    },
    {
      num: '02',
      icon: 'bi-book',
    ...
    },
    {
      ...
      desc: `Committee submits findings and recommendations to the ${labels.legislativeBody}`,
    },
    {
      num: '05',
    ...
    },
    {
      ...
      desc: `Final voting on the proposed ordinance by the ${labels.legislativeBody}`,
    },
    {
      num: '07',
      icon: 'bi-calendar-check',
      title: `10-Day ${labels.leaderTitle}'s Approval`,
      desc: `${labels.leaderTitle} reviews and approves the enacted ordinance within 10 days`,
    },
    {
      num: '08',
      icon: 'bi-send',
    ...
    },
    {
      ...
      desc: `Ordinance takes effect and is enforced within the ${labels.lguTypeLabel.toLowerCase()}`,
      final: true,
    },
  ];
  ...
  const resolutionSteps = [
    {
      ...
      desc: `Submit the proposed resolution to the ${labels.legislativeBody}`,
    },
    {
      num: '02',
      icon: 'bi-calendar-event',
      title: 'Inclusion in Session Agenda',
      desc: `Resolution is scheduled for inclusion in the ${labels.legislativeBody} session`,
    },
    {
      num: '03',
      icon: 'bi-people-fill',
  ...
  const infoCards = [
    {
      ...
      title: 'Ordinances',
      desc: `Local laws with permanent and general application that require compliance from residents and businesses within the ${labels.lguTypeLabel.toLowerCase()}.`,
    },
    {
    ...
    },
    {
      ...
      desc: `Citizens can attend ${labels.legislativeBody} sessions and participate in public hearings for proposed ordinances.`,
    },
    {
        ...
        <div className="container mx-auto px-4">
          <div className="text-center max-w-2xl mx-auto">
            <span className="inline-flex items-center gap-2 bg-white/20 text-white px-4 py-2 rounded-full text-sm font-medium mb-4">
              <i className="bi bi-bank2" /> {labels.legislativeBody}
            </span>
            <h1 className="text-3xl md:text-4xl font-bold text-white mb-4">
              Legislative Documents
            </h1>
            <p className="text-lg text-white/90">
              Ordinances and resolutions of the {labels.legislativeBody} ng{' '}
              {lguName}
            </p>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            <Link
              ...
              className="group bg-white border border-gray-200 rounded-xl p-8 no-underline transition-all duration-200 hover:border-primary-500 hover:shadow-lg"
            >
              ...
              <span className="text-primary-600 font-medium flex items-center gap-1 group-hover:gap-2 transition-all">
                Browse Ordinances <i className="bi bi-arrow-right" />
              </span>
            </Link>
            <Link
            ...
            >
              ...
              <p className="text-gray-500 mb-4">
                Resolutions passed by the {labels.legislativeBody} expressing
                the will or opinion of the legislative body on various matters.
              </p>
              <span className="text-primary-600 font-medium flex items-center gap-1 group-hover:gap-2 transition-all">
                Browse Resolutions <i className="bi bi-arrow-right" />
              </span>
            </Link>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-10">
            <span className="inline-flex items-center gap-2 bg-primary-600 text-white px-4 py-2 rounded-full text-sm font-medium mb-3">
              <i className="bi bi-diagram-3-fill" /> Process Flow
            </span>
          ...
          <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {steps.map((step) => (
              <div
                key={step.num}
                className={`bg-white border ${step.final ? 'border-green-300 bg-green-50' : 'border-gray-200'} rounded-xl p-6 relative`}
              >
                <span
                  className={`absolute -top-3 -left-3 w-8 h-8 flex items-center justify-center rounded-full text-sm font-bold ${step.final ? 'bg-green-600 text-white' : 'bg-primary-600 text-white'}`}
                >
                  {step.num}
                </span>
                <div
                  className={`w-10 h-10 flex items-center justify-center rounded-lg mb-3 ${step.final ? 'bg-green-100 text-green-600' : 'bg-primary-50 text-primary-600'}`}
                >
