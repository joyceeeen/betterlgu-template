    ...
    <>
      ...
      <div className="container mx-auto px-4">
        <nav className="py-4 text-sm text-gray-500" aria-label="Breadcrumb">
          ...
          <span className="mx-2">/</span>
          <span aria-current="page" className="text-gray-900">Ordinance Framework</span>
        </nav>
      </div>
      ...
      <section className="bg-gradient-to-br from-primary-600 to-primary-700 py-16">
        <div className="container mx-auto px-4">
          <div className="text-center max-w-2xl mx-auto">
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-white mb-4">Ordinance Framework</h1>
            <p className="text-lg text-white/90">{labels.deptPrefix} ordinances enacted by the {labels.legislativeBody} ng {lguName}</p>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto bg-white border border-gray-200 rounded-xl p-8">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-2">What is an Ordinance?</h3>
                <p className="text-gray-600 mb-4">A {labels.lguTypeLabel.toLowerCase()} ordinance is a local law enacted by the {labels.legislativeBody} ({labels.legislativeBodyAbbr}) that governs the {labels.lguTypeLabel.toLowerCase()} and its residents. Ordinances have the force and effect of law within the territorial jurisdiction of the {labels.lguTypeLabel.toLowerCase()}.</p>
                <p className="text-gray-600">Ordinances may cover various subjects including but not limited to: taxation, business regulations, public safety, environmental protection, traffic management, and zoning.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 max-w-4xl mx-auto">
            {ordinanceCategories.map((cat) => (
              <div key={cat.label} className="flex items-center gap-2 p-4 bg-white border border-gray-200 rounded-xl text-center justify-center">
                <i className={`bi ${cat.icon} text-primary-600`} /> <span className="text-sm font-medium text-gray-700">{cat.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-12">
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            <h2 className="text-xl font-bold text-gray-900 mb-2">Sample Ordinances</h2>
            <p className="text-gray-500">Sample ordinances — replace with actual data from your {labels.legislativeBody}</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full bg-white border border-gray-200 rounded-xl overflow-hidden">
              <thead className="bg-gray-50">
                <tr>
                  <th scope="col" className="px-6 py-4 text-left text-sm font-semibold text-gray-900" style={{ width: '120px' }}>Ordinance No.</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {sampleOrdinances.map((ord) => (
                  <tr key={ord.no} className="hover:bg-gray-50">
                    <td className="px-6 py-4 text-sm text-gray-900 font-medium">{ord.no}</td>
                    <td className="px-6 py-4 text-sm text-gray-600">{ord.title}</td>
                    <td className="px-6 py-4 text-sm text-gray-500">{ord.date}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="text-center mt-6">
            <p className="text-sm text-gray-500 flex items-center justify-center gap-2">
              <i className="bi bi-info-circle" /> Update this section with actual ordinances from your LGU's {labels.legislativeBody} website.
            </p>
          </div>
        </div>
      </section>
