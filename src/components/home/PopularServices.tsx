          ...
          {services.map((service) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <h3
                  className={`text-base font-semibold m-0 mb-1 ${service.isViewAll ? 'text-white' : 'text-gray-900'}`}
                >
                  {t(service.titleKey)}
                </h3>
                {service.descKey && (
                  <p
                    className={`text-[0.8125rem] m-0 ${service.isViewAll ? 'text-white/80' : 'text-gray-500'}`}
                  >
                    {t(service.descKey)}
                  </p>
                )}
                {service.isViewAll && (
                  <p className="text-[0.8125rem] m-0 text-white/80">
              ...
              {/* Arrow */}
              <i
                className={`bi bi-arrow-right transition-all duration-200 opacity-0 group-hover:opacity-100 group-hover:translate-x-1 ${
